#
# OMNeT++/OMNEST Makefile for miximexamples
#
# This file was generated with the command:
#  opp_makemake -f --deep -o miximexamples -O out -I../src/base/phyLayer -I../src -I../src/base -I../src/inet_stub/mobility -I../src/base/utils -I../src/base/modules -I../src/inet_stub/base -I../src/modules/power/battery -I../src/inet_stub/util -I../src/inet_stub/linklayer/contract -I../src/base/messages -I../src/base/connectionManager -I../src/inet_stub/mobility/models -I../src/inet_stub/networklayer/common -L../out/$$\(CONFIGNAME\)/src -lmixim -DWITH_MIXIM_EXAMPLES -DMIXIM_INET
#

# Name of target to be created (-o option)
TARGET = miximexamples$(EXE_SUFFIX)

# User interface (uncomment one) (-u option)
USERIF_LIBS = $(ALL_ENV_LIBS) # that is, $(TKENV_LIBS) $(CMDENV_LIBS)
#USERIF_LIBS = $(CMDENV_LIBS)
#USERIF_LIBS = $(TKENV_LIBS)

# C++ include paths (with -I)
INCLUDE_PATH = \
    -I../src/base/phyLayer \
    -I../src \
    -I../src/base \
    -I../src/inet_stub/mobility \
    -I../src/base/utils \
    -I../src/base/modules \
    -I../src/inet_stub/base \
    -I../src/modules/power/battery \
    -I../src/inet_stub/util \
    -I../src/inet_stub/linklayer/contract \
    -I../src/base/messages \
    -I../src/base/connectionManager \
    -I../src/inet_stub/mobility/models \
    -I../src/inet_stub/networklayer/common \
    -I. \
    -Inodes \
    -Inodes/appl \
    -Inodes/messages \
    -Inodes/netwlayer \
    -Iresults

# Additional object and library files to link with
EXTRA_OBJS =

# Additional libraries (-L, -l options)
LIBS = -L../out/$(CONFIGNAME)/src  -lmixim
LIBS += -Wl,-rpath,`abspath ../out/$(CONFIGNAME)/src`

# Output directory
PROJECT_OUTPUT_DIR = ../out
PROJECTRELATIVE_PATH = examples
O = $(PROJECT_OUTPUT_DIR)/$(CONFIGNAME)/$(PROJECTRELATIVE_PATH)

# Object files for local .cc and .msg files
OBJS = $O/nodes/appl/ARAappl.o $O/nodes/netwlayer/ARAnetwlayer.o $O/nodes/messages/ARAmessages_m.o

# Message files
MSGFILES = \
    nodes/messages/ARAmessages.msg

#------------------------------------------------------------------------------

# Pull in OMNeT++ configuration (Makefile.inc or configuser.vc)

ifneq ("$(OMNETPP_CONFIGFILE)","")
CONFIGFILE = $(OMNETPP_CONFIGFILE)
else
ifneq ("$(OMNETPP_ROOT)","")
CONFIGFILE = $(OMNETPP_ROOT)/Makefile.inc
else
CONFIGFILE = $(shell opp_configfilepath)
endif
endif

ifeq ("$(wildcard $(CONFIGFILE))","")
$(error Config file '$(CONFIGFILE)' does not exist -- add the OMNeT++ bin directory to the path so that opp_configfilepath can be found, or set the OMNETPP_CONFIGFILE variable to point to Makefile.inc)
endif

include $(CONFIGFILE)

# Simulation kernel and user interface libraries
OMNETPP_LIB_SUBDIR = $(OMNETPP_LIB_DIR)/$(TOOLCHAIN_NAME)
OMNETPP_LIBS = -L"$(OMNETPP_LIB_SUBDIR)" -L"$(OMNETPP_LIB_DIR)" -loppmain$D $(USERIF_LIBS) $(KERNEL_LIBS) $(SYS_LIBS)

COPTS = $(CFLAGS) -DWITH_MIXIM_EXAMPLES -DMIXIM_INET $(INCLUDE_PATH) -I$(OMNETPP_INCL_DIR)
MSGCOPTS = $(INCLUDE_PATH)

# we want to recompile everything if COPTS changes,
# so we store COPTS into $COPTS_FILE and have object
# files depend on it (except when "make depend" was called)
COPTS_FILE = $O/.last-copts
ifneq ($(MAKECMDGOALS),depend)
ifneq ("$(COPTS)","$(shell cat $(COPTS_FILE) 2>/dev/null || echo '')")
$(shell $(MKPATH) "$O" && echo "$(COPTS)" >$(COPTS_FILE))
endif
endif

#------------------------------------------------------------------------------
# User-supplied makefile fragment(s)
# >>>
# <<<
#------------------------------------------------------------------------------

# Main target
all: $O/$(TARGET)
	$(Q)$(LN) $O/$(TARGET) .

$O/$(TARGET): $(OBJS)  $(wildcard $(EXTRA_OBJS)) Makefile
	@$(MKPATH) $O
	@echo Creating executable: $@
	$(Q)$(CXX) $(LDFLAGS) -o $O/$(TARGET)  $(OBJS) $(EXTRA_OBJS) $(AS_NEEDED_OFF) $(WHOLE_ARCHIVE_ON) $(LIBS) $(WHOLE_ARCHIVE_OFF) $(OMNETPP_LIBS)

.PHONY: all clean cleanall depend msgheaders

.SUFFIXES: .cc

$O/%.o: %.cc $(COPTS_FILE)
	@$(MKPATH) $(dir $@)
	$(qecho) "$<"
	$(Q)$(CXX) -c $(CXXFLAGS) $(COPTS) -o $@ $<

%_m.cc %_m.h: %.msg
	$(qecho) MSGC: $<
	$(Q)$(MSGC) -s _m.cc $(MSGCOPTS) $?

msgheaders: $(MSGFILES:.msg=_m.h)

clean:
	$(qecho) Cleaning...
	$(Q)-rm -rf $O
	$(Q)-rm -f miximexamples miximexamples.exe libmiximexamples.so libmiximexamples.a libmiximexamples.dll libmiximexamples.dylib
	$(Q)-rm -f ./*_m.cc ./*_m.h
	$(Q)-rm -f nodes/*_m.cc nodes/*_m.h
	$(Q)-rm -f nodes/appl/*_m.cc nodes/appl/*_m.h
	$(Q)-rm -f nodes/messages/*_m.cc nodes/messages/*_m.h
	$(Q)-rm -f nodes/netwlayer/*_m.cc nodes/netwlayer/*_m.h
	$(Q)-rm -f results/*_m.cc results/*_m.h

cleanall: clean
	$(Q)-rm -rf $(PROJECT_OUTPUT_DIR)

depend:
	$(qecho) Creating dependencies...
	$(Q)$(MAKEDEPEND) $(INCLUDE_PATH) -f Makefile -P\$$O/ -- $(MSG_CC_FILES)  ./*.cc nodes/*.cc nodes/appl/*.cc nodes/messages/*.cc nodes/netwlayer/*.cc results/*.cc

# DO NOT DELETE THIS LINE -- make depend depends on it.
$O/nodes/appl/ARAappl.o: nodes/appl/ARAappl.cc \
	nodes/appl/ARAappl.h \
	nodes/messages/ARAmessages_m.h \
	../src/base/modules/BaseApplLayer.h \
	../src/base/modules/BaseBattery.h \
	../src/base/modules/BaseLayer.h \
	../src/base/modules/BaseModule.h \
	../src/base/modules/MiximBatteryAccess.h \
	../src/base/utils/HostState.h \
	../src/base/utils/MiXiMDefs.h \
	../src/base/utils/PassedMessage.h \
	../src/base/utils/SimpleAddress.h \
	../src/base/utils/miximkerneldefs.h \
	../src/inet_stub/base/INETDefs.h \
	../src/inet_stub/linklayer/contract/MACAddress.h
$O/nodes/messages/ARAmessages_m.o: nodes/messages/ARAmessages_m.cc \
	nodes/messages/ARAmessages_m.h \
	../src/base/utils/MiXiMDefs.h \
	../src/base/utils/SimpleAddress.h \
	../src/base/utils/miximkerneldefs.h \
	../src/inet_stub/base/INETDefs.h \
	../src/inet_stub/linklayer/contract/MACAddress.h
$O/nodes/netwlayer/ARAnetwlayer.o: nodes/netwlayer/ARAnetwlayer.cc \
	nodes/messages/ARAmessages_m.h \
	nodes/netwlayer/ARAnetwlayer.h \
	../src/base/modules/BaseBattery.h \
	../src/base/modules/BaseLayer.h \
	../src/base/modules/BaseModule.h \
	../src/base/modules/BaseNetwLayer.h \
	../src/base/modules/BaseWorldUtility.h \
	../src/base/modules/MiximBatteryAccess.h \
	../src/base/utils/FindModule.h \
	../src/base/utils/HostState.h \
	../src/base/utils/MiXiMDefs.h \
	../src/base/utils/PassedMessage.h \
	../src/base/utils/SimpleAddress.h \
	../src/base/utils/miximkerneldefs.h \
	../src/inet_stub/base/Coord.h \
	../src/inet_stub/base/INETDefs.h \
	../src/inet_stub/linklayer/contract/MACAddress.h \
	../src/inet_stub/util/FWMath.h \
	../src/modules/power/battery/BatteryState.h \
	../src/modules/power/battery/DeviceEntry.h \
	../src/modules/power/battery/SimpleBattery.h

